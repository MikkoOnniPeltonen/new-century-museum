...
test('qualified lifespans and historical sources are visible in the timeline and room', async ({ page }) => {
  await seedRooms(page, { '1700s': ['maria-merian'] })
  await page.goto('/timeline')
  const merian = page.getByRole('button', { name: /^Maria Sibylla Merian\s*1647–1717/ })
  await expect(merian).not.toHaveClass(/is-living/)
  await merian.click()
  const drawer = page.getByRole('dialog', { name: 'Maria Sibylla Merian' })
  await drawer.getByText('Sources & historical context', { exact: true }).click()
  await expect(drawer.getByText(/major 1705 publication/)).toBeVisible()
  await expect(drawer.getByRole('link', { name: /Rijksmuseum/ })).toHaveAttribute('href', 'https://id.rijksmuseum.nl/200206122')
  await page.keyboard.press('Escape')
  await page.goto('/room/1700s')
  await expect(page.getByText('1647–1717', { exact: false })).toBeVisible()
  await page.getByText('Sources & historical context', { exact: true }).click()
  await expect(page.getByRole('link', { name: /Rijksmuseum/ })).toBeVisible()
